...
export default function ExpensesSection({ state, setState, fmt }: Props) {
  const { members, expenses } = state;
  const [payerId, setPayerId] = useState("");
  const [description, setDescription] = useState("");
  const [amount, setAmount] = useState("");
  // track who is EXCLUDED rather than included, so members added later default
  // to "shares this expense" without any effect-based syncing.
  const [excluded, setExcluded] = useState<Set<string>>(new Set());

  const memberIds = useMemo(() => members.map((m) => m.id), [members]);
  // effective payer: the chosen one if still present, otherwise the first member
  ...
  const nameOf = (id: string) => members.find((m) => m.id === id)?.name ?? "(removed)";
        ...
        <div className="flex flex-col gap-3 rounded-xl border border-border bg-background p-3">
          <div className="flex flex-col gap-2 sm:flex-row">
            <label className="flex flex-col gap-1 sm:w-32">
              <span className="text-xs font-medium text-muted">Paid by</span>
              <select
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between">
              <span className="text-xs font-medium text-muted">Split between</span>
              <button
                type="button"
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-muted">paid for</span>
                  <span className="truncate">{e.description}</span>
                </div>
                <div className="mt-0.5 text-xs text-muted">
                  {e.participantIds.length === members.length
                    ? "Split between everyone"
                    : `Split between ${e.participantIds.map(nameOf).join(" · ")}`}
                </div>
              </div>
